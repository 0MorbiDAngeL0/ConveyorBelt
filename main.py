...
# ekran ölçeği
CELLS_LOAD = 60
CELLS_L1   = 30   # YENİ: Line-1 görsel uzunluğu (hücre)
CELLS_BELT = 18
CELLS_LINE2= 18
CELLS_UNL  = 14
...
class System:
    def __init__(self):
        ...
        # Yükleme halkası (LOAD) ve SW sonrası hatlar
        self.load_loop: List[Moving] = []
        # YENİ: Line-1
        self.line1: List[Moving] = []
    ...
    def step_load_loop(self):
        ...
        for m in self.load_loop:
            ...
            if crossed:
                arrived.append(m)
        if arrived:
            # LOAD'tan çıkanları L1'in başına al (SW → L1)
            # Pos'a göre sırala ki gerçek sıralama korunsun
            arrived.sort(key=lambda m: m.pos)
            for m in arrived:
                self.line1.append(Moving(m.id, "L1", 0.0, LINE1_LEN_M, self.speed_for("L1")))
            # halkadan çıkar
            ids = set(x.id for x in arrived)
            self.load_loop = [m for m in self.load_loop if m.id not in ids]

    def step_line1(self):
        """Line-1 akar; sonuna ulaşanlar bantlara round-robin dağılır."""
        if not self.line1:
            return
        sp = self.speed_for("L1")
        # basit akış (istersen GAP ekleyip L1 üzerinde tampon da yapabiliriz)
        for m in self.line1:
            m.speed = sp
            m.step(DT)
        done = [m for m in self.line1 if m.done]
        if done:
            # round-robin bant seçimi ve banda giriş
            done.sort(key=lambda m: m.id)  # stabil
            for m in done:
                b = SERP_ORDER[self.rr % (BELTS_R*BELTS_C)]
                self.rr += 1
                self.belts[b].append(Moving(m.id, f"B{b}", 0.0, BELT_LEN_M, self.speed_for("B")))
            # L1'den çıkart
            ids = set(x.id for x in done)
            self.line1 = [m for m in self.line1 if m.id not in ids]

    def step_belts(self):
        """Bantlarda konvoy hareketi; COLLECT'te sonda park, DRAIN'de L2'ye düş."""
        sp = self.speed_for("B")
    ...
    def tick(self):
        self.t += DT
        self.spawn()
        self.step_load_loop()   # LOAD -> L1
        self.step_line1()       # L1 -> Bx
        self.step_belts()       # Bx -> L2 (DRAIN'de)
        self.step_line2()       # L2 -> U1/U2
        self.step_unloads()     # U1/U2 -> done

    def snapshot(self):
        inflight = len(self.load_loop) + len(self.line1)
        belts_q  = sum(len(self.belts[b]) for b in self.belts)
        return {
            "mode": self.mode, "t": self.t,
            "inflight": inflight, "swap": 0,
...
def draw_bar_v(win, y, x, length_cells, items_cells_positions):
    """Dikey bar: her satırda en fazla 1 parça; çakışanları yukarı kaydır."""
    maxy, maxx = win.getmaxyx()
    if x < 0 or x >= maxx:
        return
...
def layout_and_draw(stdscr, sys: System):
    ...
    s = sys.snapshot()
    safe_addstr(stdscr, 0, 0, f"[{s['mode']}] t={s['t']:.1f}s  inflight={s['inflight']} belts={s['belts']} line2={s['line2']} U1={s['u1']} U2={s['u2']} done={s['done']}  (R=Drain, Q=Quit)")

    ox, oy = 2, 2

    # TEK UZUN YÜKLEME BANDI
    y = oy+2
    xs = [clamp(int((m.pos / LOAD_LOOP_LEN_M) * (CELLS_LOAD-2)), 0, CELLS_LOAD-2) for m in sys.load_loop]
    ...
    # YENİ: LINE-1 (LOAD ile bantlar arasında)
    y_l1 = y + 3
    xs_l1 = [clamp(int((m.pos / LINE1_LEN_M) * (CELLS_L1-2)), 0, CELLS_L1-2) for m in sys.line1]
    draw_bar_h(stdscr, y_l1, ox, CELLS_L1, xs_l1)
    safe_addstr(stdscr, y_l1+1, ox + CELLS_L1//2 - 2, "L1")
